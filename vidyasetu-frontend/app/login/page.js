...
import { GraduationCap, ArrowRight, UserCheck, Shield, Sparkles, User, BookOpen } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleSocialLogin = async (selectedProvider) => {
    ...
    try {
      ...
      setError(requestError.response?.data?.message || 'Authentication failed. Please verify the email.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative overflow-hidden rounded-3xl border border-slate-200/80 bg-white p-8 shadow-panel backdrop-blur sm:p-10">
        ...
        <div className="relative z-10 space-y-6">
          ...
          <p className="text-sm leading-relaxed text-slate-600">
            Sign in to access your enrolled mock series, review diagnostic scorecards, or manage test packages in the Content Studio.
          </p>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] text-slate-400">
                Choose a demo persona below to quick-fill
              </span>
            </div>
            <input
              type="email"
              ...
              className="w-full rounded-2xl border border-slate-200 bg-slate-50/90 px-4 py-3 text-sm text-slate-900 placeholder-slate-400 outline-none transition-all focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10"
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            >
              ...
              <span>{loading && provider === 'google' ? 'Authenticating...' : 'Sign in with Google'}</span>
            </button>
            ...
            >
              ...
              <span>{loading && provider === 'facebook' ? 'Authenticating...' : 'Sign in with Facebook'}</span>
            </button>
          </div>
          ...
          <div className="mt-8 rounded-2xl border border-indigo-100 bg-indigo-50/60 p-5 space-y-3">
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-indigo-800">
              <UserCheck className="h-4 w-4 text-indigo-600" />
              <span>Instant Test Personas (One-Click Fill)</span>
            </div>
            <p className="text-xs text-slate-600 leading-relaxed">
              Click any profile below to automatically populate the email field and sign in directly:
            </p>
            ...
            <div className="grid gap-2 sm:grid-cols-1">
              {demoAccounts.map((account) => (
                <button
                  ...
                  className={`flex flex-col sm:flex-row sm:items-center justify-between rounded-xl border p-3 text-left transition-all ${
                    email === account.email
                ...
                >
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-slate-900">{account.title}</span>
                  ...
                  <span className="text-xs font-mono text-indigo-600 mt-1 sm:mt-0">{account.email}</span>
                </button>
              ))}
            </div>
          </div>
